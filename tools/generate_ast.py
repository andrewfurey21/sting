...
def define_ast(output_dir:str, base_name:str, types:List[str]):
    ...
    with open(path, "w") as f:
        ...
        f.write("from typing import Protocol, TypeVar, Optional\n")
        f.write("T = TypeVar('T', covariant=True)\n\n")
        f.write("from main import Token\n")

        f.write(f"class {base_name.capitalize()}():\n")
        f.write(f"  def accept(self, visitor:Operation[T]) -> Optional[T]:\n    pass\n\n")
...
def define_operation(f:TextIOWrapper, base_name:str, types:List[str]):
    ...
    for type in types:
        ...
        f.write(f"  def {fn_name(type_name, base_name)}(self, {base_name}:{type_name}) -> Optional[T]:\n    pass\n\n")
